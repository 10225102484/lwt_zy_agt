...
def parse_vlm_result(raw: Any) -> V20VLMResult | None:
    ...
    plan_source = payload.get("steps") if isinstance(payload.get("steps"), list) else payload.get("plan")
    plan_steps = [dict(x) for x in plan_source[:24] if isinstance(x, dict)] if isinstance(plan_source, list) else []
    plan_steps = [x for x in plan_steps if not _is_schema_echo_plan_step(x)]
...
class MyAgent(_BaseAgent):
    ...
    def _minor_structural_churn(self, report: TransitionReport) -> bool:
    ...
    def _actor_merged_into_container(self, pending: PendingAction, current_actor: str) -> bool:
        old_obj = pending.scene_before.object_by_id(current_actor)
        if old_obj is None or old_obj.frame_color is not None:
            return False
    ...
    def _enforce_transition_evidence_contract(self, pending: PendingAction, report: TransitionReport, current_scene: SceneSnapshot, outcome: str) -> None:
        ...
        if current_actor and current_actor in disappeared:
            ...
            merged = (
                ...
                and report.life_delta in (None, 0)
                and self._actor_merged_into_container(pending, current_actor)
            )
    ...
    def _transition_matches_expected(self, report: TransitionReport, step: PlanStep) -> bool:
        ...
        if not preds:
            return bool(
                ...
                and report.life_delta in (None, 0)
                and not self._minor_structural_churn(report)
            )
        if self._minor_structural_churn(report):
            return False
    ...
    def _transition_progress_score(self, pending: PendingAction, report: TransitionReport, current_scene: SceneSnapshot) -> float:
        ...
        if step is not None and pending.source == "plan_executor":
            if self._transition_matches_expected(report, step):
                score += 1.5
            else:
                score -= 0.8
        return score
    ...
    def _update_plan_after_transition(self, pending: PendingAction, report: TransitionReport, current_scene: SceneSnapshot) -> None:
        ...
        if target_id and self._target_reached(target_id, current_scene):
            ...
            level.plan_cursor += 1
            return
        matched = self._transition_matches_expected(report, step)
        if pending.source == "plan_executor":
            ...
            if not report.effective_noop:
                ...
                self._contract_forbid_action(
                    pending.scene_before,
                    pending.action_key(),
                    "plan_step_expected_predicate_mismatch",
                )
                return
    ...
    def _update_action_meaning_from_transition(self, pending: PendingAction, report: TransitionReport, current_scene: SceneSnapshot) -> None:
        ...
        if move is not None:
            ...
            if side_effect and not report.is_simple_translation:
                ...
                pure_movement_votes = max(0, meaning.movements - meaning.transforms - meaning.interactions)
                if dominant is not None and dominant[1] >= 2 and pure_movement_votes >= 1 and meaning.noop_ratio < 0.35:
    ...
    def _refresh_strategic_flags(self, scene: SceneSnapshot, transition: TransitionReport | None) -> None:
        ...
        if counter_low:
            ...
            if level.bottleneck_reason in {"low_resource_counter", "critical_resource_counter", "resource_crisis_no_safe_fallback"}:
                level.bottleneck_reason = ""
            self.logger.log_event("resource_crisis_cleared_v20", {"level": level.level_index, "state": scene.state_hash[:12], "counter": scene.counter_value, "ratio": scene.counter_ratio, "lives": scene.life_count})
    ...
    def _click_step_has_executable_candidate(self, step: PlanStep, scene: SceneSnapshot) -> bool:
        ...
        if coords:
            if any(
                ...
                return True
            # All explicit coordinates are exhausted/banned. Instead of dropping the
            # whole VLM step (which starves the executor and forces another VLM call),
            # keep it and let _proposal_for_step remap onto a fresh point; the normal
            # per-coordinate/object caps still apply to the remapped click.
            ...
            if self._first_untried_action6_target(scene, blocked) is not None:
    ...
    def _transform_controller_action(self, scene: SceneSnapshot, legal: Sequence[Any]) -> tuple[Any, dict[str, Any], str] | None:
        ...
        for name in simple_action_names(legal):
            ...
            score = 0.0
            if m:
                score += 0.2 * min(m.transforms, 2) + 1.8 * m.interactions - 2.2 * m.noop_ratio - 2.0 * (m.kind == "undo")
            ...
            if progress_samples:
                ...
                if avg_progress <= 0.0 and counts.get("transform", 0) >= 2:
                    score -= 3.0
            score += 0.1 * min(counts.get("transform", 0), 2) - 0.18 * max(0, counts.get("transform", 0) - 2) + 0.9 * counts.get("interaction", 0) - 1.0 * counts.get("noop", 0)
    ...
    def _fallback_nonreset_action(self, scene: SceneSnapshot | None, legal: Sequence[Any], purpose: str, *, allow_guard: bool = True) -> tuple[Any, dict[str, Any], str] | None:
        ...
        safe_click_fuse_cooling = level.click_fuse_block_until > level.total_action_count and not self._click_only_legal(legal)
        if "ACTION6" in {action_name(a).upper() for a in legal} and not safe_click_fuse_cooling:
            picked = self._first_untried_action6_target(scene, blocked)
            if picked is not None:
                x, y, target_id = picked
        ...
        click_fuse_cooling = level.click_fuse_block_until > level.total_action_count and not self._click_only_legal(legal)
        for action in legal:
            name = action_name(action).upper()
            if name == "RESET" or self._action_blocked(scene, name, ignore_recent=True):
                continue
            if name == "ACTION6":
                if click_fuse_cooling:
                    continue
    ...
    def _evidence_exhausted_nonreset_action(self, scene: SceneSnapshot, legal: Sequence[Any], purpose: str) -> tuple[Any, dict[str, Any], str] | None:
        ...
        def rank(item: tuple[Any, str]) -> tuple[float, float, float]:
            action, name = item
            ...
            return (danger, float(recent_usage.get(name, 0)), float(action_value(action)))
